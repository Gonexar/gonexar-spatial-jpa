...
interface MatrixOperator {
    ...
    fun SpatialExpr<Raster>.stSummaryStats(
        ...
        val min =
            dsl.register("${alias}_min", dsl.cb.function("st_column", Double::class.java, stats, dsl.cb.literal(1)))
        val max =
            dsl.register("${alias}_max", dsl.cb.function("st_column", Double::class.java, stats, dsl.cb.literal(2)))
        val mean =
            dsl.register("${alias}_mean", dsl.cb.function("st_column", Double::class.java, stats, dsl.cb.literal(3)))
        val std =
            dsl.register("${alias}_std", dsl.cb.function("st_column", Double::class.java, stats, dsl.cb.literal(4)))
        val count =
            dsl.register("${alias}_count", dsl.cb.function("st_column", Long::class.java, stats, dsl.cb.literal(5)))
        val sum =
            dsl.register("${alias}_sum", dsl.cb.function("st_column", Double::class.java, stats, dsl.cb.literal(6)))
